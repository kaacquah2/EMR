...
export async function login(page: Page, creds: E2ECredentials): Promise<boolean> {
  ...
  if (mfaVisible) {
    ...
    if (backup) {
      ...
      throw new Error(
        "Login requires MFA. Set E2E_MFA_BACKUP_CODE or use a test user with MFA disabled."
      );
    }
  }
...
export function getPharmacyCreds(): E2ECredentials | null {
  const email = process.env.E2E_PHARMACY_EMAIL;
  const password = process.env.E2E_PHARMACY_PASSWORD;
  if (!email || !password) return null;
  return { email, password };
}
