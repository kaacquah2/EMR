    ...
    return (
      <div className="flex min-h-[200px] items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-2 text-sm text-slate-500 dark:text-slate-500">You do not have permission to access this page.</p>
        </div>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      {needsWorkflowSetup && (
        <Card className="border-[#0EAFBE]/30 bg-[#F0FDFF] p-4">
          <p className="text-sm font-medium text-slate-900 dark:text-slate-100">Workflow setup</p>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-slate-200 dark:border-slate-800">
                ...
                <th className="px-4 py-2 text-left text-xs font-semibold text-slate-500 dark:text-slate-500">Email</th>
                <th className="px-4 py-2 text-left text-xs font-semibold text-slate-500 dark:text-slate-500">Role</th>
                <th className="px-4 py-2 text-left text-xs font-semibold text-slate-500 dark:text-slate-500">Department / Unit</th>
                <th className="px-4 py-2 text-left text-xs font-semibold text-slate-500 dark:text-slate-500">GMDC Licence</th>
            ...
            <tbody>
              {loading ? (
                <tr>
                  <td colSpan={7} className="px-4 py-8 text-center text-slate-500 dark:text-slate-500">Loading...</td>
                </tr>
              ) : users.length === 0 ? (
                <tr>
                  <td colSpan={7} className="px-4 py-8 text-center text-slate-500 dark:text-slate-500">No users.</td>
                </tr>
              ) : (
                users.map((u) => (
                  <tr key={u.user_id} className="border-b border-slate-100 dark:border-slate-900">
                    <td className="px-4 py-2">{u.full_name}</td>
                    <td className="px-4 py-2">{u.email}</td>
                    <td className="px-4 py-2">
                      <span className="rounded-full bg-slate-100 dark:bg-slate-900 px-2 py-0.5 text-xs">{u.role}</span>
                    </td>
                          ...
                          <>
                            {user?.role === "hospital_admin" &&
                              ...
                              <div className="flex items-center gap-1">
                                <select
                                  className="h-8 rounded border border-slate-300 dark:border-slate-700 px-2 text-xs"
                                  value={u.role}
                                  onChange={(e) => {
      ...
      <SlideOver open={inviteOpen} onOpenChange={setInviteOpen} title={isSuperAdmin ? "Create Hospital Admin" : "Invite User"}>
        ...
        >
          ...
          {isSuperAdmin && (
            <div>
              <label className="mb-1.5 block text-xs font-semibold uppercase text-slate-500 dark:text-slate-500">Hospital</label>
              <select
              ...
              <p className="mt-1 text-xs text-slate-500 dark:text-slate-500">This admin will manage only this facility.</p>
            </div>
          )}
          {!isSuperAdmin && (
            <>
              <div>
                <label className="mb-1.5 block text-xs font-semibold uppercase text-slate-500 dark:text-slate-500">Role</label>
                <select
              ...
              <div>
                <label className="mb-1.5 block text-xs font-semibold uppercase text-slate-500 dark:text-slate-500">Department / Unit</label>
                <select
                ...
                <p className="mt-1 text-xs text-slate-500 dark:text-slate-500">Assign staff to a department so work is routed to the correct team.</p>
              </div>
              ...
              {inviteForm.role === "lab_technician" && (
                <div>
                  <label className="mb-1.5 block text-xs font-semibold uppercase text-slate-500 dark:text-slate-500">Lab Unit (service tag)</label>
                  <select
                  ...
                  <p className="mt-1 text-xs text-slate-500 dark:text-slate-500">Technician will only see orders for this unit (e.g. Hematology, Chemistry).</p>
                </div>
              )}
      ...
      <SlideOver open={bulkOpen} onOpenChange={(open) => { setBulkOpen(open); if (!open) setBulkResult(null); setBulkFile(null); }} title="Bulk CSV Import">
        ...
        />
        {bulkResult && (
          <div className="mb-4 rounded-lg border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900 p-3 text-sm">
            <p className="font-medium text-slate-900 dark:text-slate-100">Created: {bulkResult.created}</p>
          ...
          <DialogContent>
            ...
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 rounded-lg border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900 px-3 py-2 text-sm text-slate-900 dark:text-slate-100"
              />
              <Button
                type="button"
                variant="secondary"
