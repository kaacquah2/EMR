...
| 3 | All required secrets set (no CI fallbacks on deploy branches) | DevOps | ☐ |
| 4 | Argon2 password hashing active (`PASSWORD_HASHERS`) | Backend | ☑ |
| 5 | JWT rotation + short access token lifetime enforced | Backend | ☐ |
| 6 | Anomaly detection uses shared Redis (not per-worker memory) | Backend | ☑ |
| 7 | `LLM_MODE=bedrock` in production (mock guard passes) | Backend | ☑ |
| 8 | Django admin on non-guessable URL or disabled; IP/VPN restricted | Security | ☐ |
| 9 | TLS termination (HTTPS only); HSTS enabled | DevOps | ☐ |
| 10 | CORS locked to known frontend origins (no `*`) | Backend | ☐ |
| 11 | Redis `requirepass` + TLS (`rediss://`) in production | DevOps | ☐ |
| 12 | Flower behind basic auth (not public :5555) | DevOps | ☑ |
...
| 24 | Celery worker + beat supervised (`supervisord` or platform) | DevOps | ☑ |
...
*Last updated: production hardening sprint. Reconcile ☑ items after each release.*
