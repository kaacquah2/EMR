...
def _hospitals_list_data():
    ...
    facility_patients = FacilityPatient.objects.filter(deleted_at__isnull=True).values(
        'facility_id').annotate(count=Count('global_patient_id', distinct=True))
    ...
    data = []
    for h in hospitals:
        # All checks now come from already-annotated counts (dynamic attributes via .annotate())
        checks = [
            h.ward_count > 0,  # pyright: ignore[reportAttributeAccessIssue]
            h.department_count > 0,  # pyright: ignore[reportAttributeAccessIssue]
            h.lab_unit_count > 0,  # pyright: ignore[reportAttributeAccessIssue]
        ...
        data.append(
            {
                ...
                "user_count": h.staff_count,  # pyright: ignore[reportAttributeAccessIssue]
                "staff_count": h.staff_count,  # pyright: ignore[reportAttributeAccessIssue]
                "patient_count": patient_counts.get(h.id, 0),
                "hospital_admin_count": h.hospital_admin_count,  # pyright: ignore[reportAttributeAccessIssue]
...
def global_audit_logs(request):
    ...
    return Response({
        "data": [
            {
                "log_id": str(log.pk),  # Use pk instead of id for type safety
...
def break_glass_list_global(request):
    ...
    filters: dict[str, Any] = {"created_at__gte": cutoff}
    ...
    data = [
        {
            "break_glass_id": str(log.pk),  # Use pk for type safety
            ...
            "hospital_id": str(log.facility.pk) if log.facility else "",  # Access via FK object
            "patient_name": log.global_patient.full_name if log.global_patient else "",
            "global_patient_id": str(log.global_patient.pk) if log.global_patient else "",  # Access via FK object
            "reason": log.reason,
            "created_at": log.created_at.isoformat(),
            "reviewed": log.reviewed,
            "excessive_usage": log.excessive_usage,
        }
        for log in logs
    ]
...
def gmdc_unverified_doctors(request):
    ...
    users = User.objects.filter(
        role="doctor", licence_verified=False).exclude(
        gmdc_licence_number__isnull=True).exclude(
            gmdc_licence_number="").select_related("hospital").order_by("full_name")[
                :100]
    data = [
        {
            "user_id": str(u.id),
...
def grant_hospital_access(request):
    ...
    audit_log(
        ...
        resource_id=access.id,  # pyright: ignore[reportAttributeAccessIssue]
        hospital=hospital,
        request=request,
...
def hospital_onboarding_dashboard(request):
    ...
    for hospital in hospitals:
        ...
        facility_links = FacilityPatient.objects.filter(
            facility=hospital, deleted_at__isnull=True).values_list(
            'global_patient', flat=True).distinct().count()
        ...
        data.append({
            ...
            "created_at": hospital.created_at.isoformat() if hospital.created_at else None,  # pyright: ignore[reportAttributeAccessIssue]
            "staff": {
                "total": user_count,
...
def bulk_import_staff(request, hospital_id):    # sourcery skip: low-code-quality
    ...
    try:
        ...
        created = []
        errors = []
        idx = 1  # Initialize idx in case CSV is empty
        ...
        for idx, row in enumerate(reader, start=2):
            try:
                ...
                if ward_name and role in ["nurse", "doctor"]:
                    if ward := Ward.objects.filter(
                        hospital=hospital, ward_name__icontains=ward_name
                    ).first():
                        user.ward = ward
                        user.save()
...
def _compliance_alerts_data():
    ...
    with contextlib.suppress(Exception):
        ...
        if (
            bg_counts := BreakGlassLog.objects.filter(
    ...
    pending_ref = Referral.objects.filter(
        created_at__lt=cutoff_7d_ref).exclude(
        status__in=[
            "accepted",
            "completed"]).count()
    if pending_ref > 0:
...
def compliance_alerts(request):
    ...
    return denied or Response({"data": _compliance_alerts_data()})
...
def _global_audit_logs_preview(limit: int = 5):
    ...
    return [
        {
            "log_id": str(log.id),  # pyright: ignore[reportAttributeAccessIssue]
            "user": log.user.full_name,
            "action": log.action,
            "resource_type": log.resource_type,
            "timestamp": log.timestamp.isoformat(),
...
def _pending_admin_grants_payload(request):
    ...
    return {
        ...
        "pending_invites": [
            {
                "user_id": str(u.id),
                "email": u.email,
                "hospital_id": str(u.hospital_id) if u.hospital_id else None,  # pyright: ignore[reportAttributeAccessIssue]
                "hospital_name": u.hospital.name if u.hospital else None,
        ...
        "pending_grants": [
            {
                "access_id": str(a.id),  # pyright: ignore[reportAttributeAccessIssue]
                "super_admin_email": a.super_admin.email if a.super_admin else "",
                "hospital_id": str(a.hospital_id),  # pyright: ignore[reportAttributeAccessIssue]
...
def pending_admin_grants(request):
    ...
    """
    denied = _require_super_admin(request)
    return denied or Response(_pending_admin_grants_payload(request))
