"""
RBAC matrix, DRF helpers, and permission enforcement middleware.
Single source of truth for endpoint-level permissions (``PERMISSION_MATRIX``).
"""
import logging
...
PERMISSION_MATRIX = {
    ...
    "auth/passkey/auth/begin": {"public": ["POST"]},
    "auth/passkey/auth/complete": {"public": ["POST"]},
    "auth/passkeys": {"authenticated": ["GET"]},
    "auth/passkeys/<uuid:pk>": {"authenticated": ["DELETE", "POST"]},
    "auth/passkeys/<uuid:pk>/rename": {"authenticated": ["POST"]},
    # Admin passkey management endpoints
    "admin/users/<uuid:user_id>/passkeys": {
        "super_admin": ["GET"],
        "hospital_admin": ["GET"],
    },
    "admin/users/<uuid:user_id>/passkeys/reset": {
        "super_admin": ["POST"],
        "hospital_admin": ["POST"],
    },
    # Patient endpoints
    ...
    "patients/<pk>/encounters/<pk>/draft": {
        "doctor": ["GET", "PATCH", "DELETE"],
        "hospital_admin": ["GET", "PATCH", "DELETE"],
        "super_admin": ["GET", "PATCH", "DELETE"],
    },
    "patients/<pk>/draft": {
        "doctor": ["GET", "PATCH", "DELETE"],
        "hospital_admin": ["GET", "PATCH", "DELETE"],
    ...
    "lab/results": {"lab_technician": ["GET"], "super_admin": ["GET"]},
    "shifts/start": {
        "nurse": ["POST"],
        "lab_technician": ["POST"],
        "doctor": ["POST"],
    ...
    "shifts/schedule/<pk>": {"hospital_admin": ["PATCH"], "super_admin": ["PATCH"]},
    "shifts/schedule/<pk>/delete": {"hospital_admin": ["DELETE"], "super_admin": ["DELETE"]},
    "shifts/check-conflict": {"hospital_admin": ["POST"], "super_admin": ["POST"]},
    "shifts/overtime-report": {"hospital_admin": ["GET"], "super_admin": ["GET"]},
    "batch-import": {"hospital_admin": ["POST"], "super_admin": ["POST"]},
    "batch-import/<pk>": {"hospital_admin": ["GET"], "super_admin": ["GET"]},
    "batch-import/<pk>/items": {"hospital_admin": ["POST"], "super_admin": ["POST"]},
    "batch-import/<pk>/export": {"hospital_admin": ["GET"], "super_admin": ["GET"]},
    "bulk-invitations": {"hospital_admin": ["POST"], "super_admin": ["POST"]},
    "bulk-invitations/<pk>": {"hospital_admin": ["GET"], "super_admin": ["GET"]},
    "bulk-invitations/expiration-check": {"super_admin": ["GET"]},
    "bulk-invitations/send-reminders": {"super_admin": ["POST"]},
    "bulk-invitations/reminder-schedule": {"hospital_admin": ["GET"], "super_admin": ["GET"]},
    ...
    "nurse/shift-handover/<pk>/acknowledge": {"nurse": ["POST"], "super_admin": ["POST"]},
    ...
    def __call__(self, request):
        ...
        if endpoint:
            ...
            if getattr(settings, "DEBUG", False) and user and getattr(user, "is_authenticated", False):
                ...
                if role == "super_admin":
                    # RBAC-12: Super Admin bypass with auditing
                    allowed = True
                    if not self.validator.can_access(role, endpoint, request.method):
                        # Only log if they wouldn't have had access normally
                        self.validator.log_permission_denial(
                            request,
                            endpoint,
                            f"Super Admin OVERRIDE access for {request.method} {endpoint}",
                            extra_data={"event_type": "super_admin_bypass"}
                        )
                else:
                    allowed = self.validator.can_access(role, endpoint, request.method)
            if not allowed:
